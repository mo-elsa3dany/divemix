export function ambientPressureATA(depth_m: number){ return depth_m/10 + 1; }
export function modMeters(fo2: number, ppo2_limit: number){
  if (fo2 <= 0) return 0; return Math.max(0, (ppo2_limit/fo2 - 1) * 10);
}
export function ppo2AtDepth(fo2: number, depth_m: number){ return fo2 * ambientPressureATA(depth_m); }
...
export function turnPressureBar(cyl_volume_l: number, start_bar: number, gas_needed_l: number, reserve_bar: number){
...
export function eadMeters(depth_m: number, fo2: number){
  const fn2 = 1 - fo2; return ((depth_m + 10) * (fn2 / 0.79)) - 10;
}
