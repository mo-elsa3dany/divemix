  ...
  return (
    <main className="mx-auto max-w-3xl p-6 space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="space-y-1">
          RMV / SAC (L/min @ surface)
          <input
            className="w-full border rounded px-3 py-2"
            type="number"
            value={rmv}
            onChange={(e) => setRmv(+e.target.value)}
          />
        </label>
        <label className="space-y-1">
          Cylinder size (L)
          <input
            ...
            type="number"
            value={cylVol}
            onChange={(e) => setCylVol(+e.target.value)}
          />
        </label>
        <label className="space-y-1">
          Start pressure (bar)
        ...
        <label className="space-y-1">
          ...
          <input
            ...
            type="number"
            value={reserveBar}
            onChange={(e) => setReserveBar(+e.target.value)}
          />
        </label>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded p-4">
          ...
          <p>
            NDL (approx): <b>{ndl} min</b> {warnNDL && '⚠️ exceeds NDL'}
          </p>
          <p>
            MOD: <b>{mod.toFixed(1)} m</b> {warnMOD && '⚠️ beyond MOD'}
          </p>
          <p>
            PPO₂ at depth: <b>{ppo2.toFixed(2)} ATA</b>
          </p>
          <p>
            EAD (N₂-only): <b>{ead.toFixed(1)} m</b>
          </p>
        </div>
        <div className="border rounded p-4">
          <h3 className="font-medium mb-2">Gas Planning</h3>
          <p>
            Gas needed (rough): <b>{gasL.toFixed(0)} L</b>
          </p>
          <p>
            Turn pressure (est.): <b>{turnBar} bar</b>
          </p>
      ...
      <p className="text-xs text-gray-500">
        Disclaimer: Not a substitute for training or a dive computer.
      </p>
    </main>
  );
}
